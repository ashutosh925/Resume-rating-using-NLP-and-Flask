...
class Engine:
    def calculateLeadershipScore(linkedInList):
        ...
        train=[['leader','guid','lead','leadership','influenced', 'wisdom', 'inspiration', 'passion', 'drive', 'power', 'knowledge','manager','inspired','active','decisions'
               'credibility', 'energy', 'foresight', 'sensitivity', 'charisma', 'action', 'perseverance', 'uniting','like','encourage','organized','manage','grow','events','happy','innovative']
               ]
    ...
    def RolesAndResponsibility(experienceList):
      ...
      list1=['Software Trainee', 'Junoir Developer','Software Associate','Software Analyst','Software Engineer','Senior Software Analyst','Software Quality Engineer'
             ,'System Administrator','Senior Process Analyst','Software Test Engineer','Project Manager','Software Application Engineer','Senior Software QA Engineer'
             ,'Senior Software Developer','Senior Software Engineer','Senior Software Programmer','Lead Software Project Engineer','Senior Project Manager','Project Head','CTO','CEO']
      score=0
      print('Go forloop')
      for string1 in experienceList:
        ...
        if len(res)>0:
          score=score+list1.index(res[len(res)-1]) +1
    ...
    def scoreCalculator(listOfResumes):
      ...
      df={'Id':[],'Name':[],'SkillCount':[],'TechCount':[],'Leadership':[],'RolesAndResponsibility':[],'FinalScore':[]}
      df=pd.DataFrame(df)
      #print(listOfResumes)
      #print("##############33")
      l1=listOfResumes
      ...
      df['RolesAndResponsibility']=Engine.RolesAndResponsibility(l1['experience'])
      print("Hiii",df['RolesAndResponsibility'])
      #normailse scores
      ...
      #emp_dict = {'score': temp_list, 'paths': path_id}
      df['FinalScore'] =df['SkillCount']+df['TechCount']+df['Leadership']+df['RolesAndResponsibility']

      return  df


    def getCall(linkedJsonFileName,url):
        ...
        rs = {'Id': ['1'], 'Name': ['Meghna Lohani'], 'skills': listskills, 'Languages': listLanguage, 'description': listDesp,
              'experience': listExp}
